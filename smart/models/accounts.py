...
from smart.common.rdf_tools import util
from smart.common.rdf_tools.util import Literal, URIRef
...
class Account(Principal):
    ...
    account = models.OneToOneField(
        Principal, primary_key=True, parent_link=True)
    ...
    state = models.CharField(
        max_length=50, choices=STATES, default=UNINITIALIZED)
    ...
    def send_welcome_email(self):
        subject = utils.render_template_raw(
            'email/welcome/subject', {'account': self}, type='txt').strip()
        body = utils.render_template_raw(
            'email/welcome/body',
            {
            'account': self,
        ...
        utils.send_mail(
            subject, body, settings.EMAIL_FROM_ADDRESS, [self.contact_email])
    ...
    def _add_password_auth_system(self, username):
        self.auth_systems.get_or_create(
            auth_system=AuthSystem.PASSWORD(), username=username)
    ...
    def to_rdf(self, model=None):
        from smart.common.rdf_tools.util import sp, foaf, rdf
        ...
        n = URIRef(
            "%s/users/%s" % (settings.SITE_URL_PREFIX, self.email.encode()))
    ...
    def password_set(self, new_password):
        ...
        # compute the hash
        self.password_hash = self.compute_hash(
            new_password, self.password_salt)
